...
## Команды
- [x] `cd <path>` -- перейти в указанный путь;
- [x] `ls <path>` -- показать содержимое текущей директории;
...
- [x] `cvs merge <file> <index-1> <index-2> <strategy>` -- объединение разных ревизий одного файла;
...
- [x] `exit` -- завершить работу с менеджером и выгрузить изменения на диск;
    * старый контент будет скопирован в папку .fm_old, расположенную в рут-директории менеджера, из соображений безопасности (а вдруг что)
- [x] `help` -- показать команды файл-менеджера.